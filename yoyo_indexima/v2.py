...
def create_migration_table(backend: DatabaseBackend):
...
def create_log_table(backend: DatabaseBackend):
    backend.execute(f"drop table if exists {backend.log_table_quoted}")
    backend.execute(
        f"CREATE TABLE {backend.log_table_quoted} ( "
        "id STRING, "
        "migration_hash STRING, "
        "migration_id STRING, "
        "operation STRING, "
...
def create_version_table(backend: DatabaseBackend):
    backend.execute(f"drop table if exists {backend.version_table_quoted}")
    backend.execute(
        f"CREATE TABLE {backend.version_table_quoted} ("
        "version INT, "
